...
- `BacktestSpec` YAMLs under `reference_specs/sp500/`; demo notebook under `implementations/sp500/` with `DartsAutoARIMAPredictor` as first baseline.
...
- Demo notebook(s) inside `implementations/sp500/`.
...
- Demo notebook under `implementations/energy_prices/` — runs `DartsAutoARIMAPredictor` (with and without futures covariates) on WTI spot as the primary target. The notebook can be short; the S&P 500 notebook is the narrative reference.
...
- `SeasonalNaivePredictor` in `aieng/forecasting/methods/baselines/naive.py`
...
- **Apr 16, 2026 — Numerical methods packaging cleanup completed:** `DartsAutoARIMAPredictor` moved from inline notebook code to `aieng/forecasting/methods/numerical/darts_arima.py`; demo notebook updated to import it.
