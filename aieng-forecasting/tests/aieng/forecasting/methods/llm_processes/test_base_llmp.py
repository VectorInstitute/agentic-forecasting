"""Tests for ``methods.base_llmp``.

We exercise the pure helpers directly and mock ``_sample_trajectories`` (the
single LLM-call seam) to run the end-to-end predict path without any network
traffic.
"""
...
from aieng.forecasting.methods.llm_processes.base_llmp import (
    BaseLLMPredictor,
    BaseLLMPredictorConfig,
    _build_user_prompt,
    _quantiles_per_step,
    _serialize_history,
    _stack_trajectories,
...
def test_serialize_history_is_deterministic_and_parseable() -> None:
    """``_serialize_history`` renders one ISO-month line per row at fixed precision."""
    df = pd.DataFrame(
        {
    ...
    rendered = _serialize_history(df, precision=2)
    assert rendered == "2020-01: 100.12\n2020-02: 101.46\n2020-03: 102.79"
    assert _serialize_history(df, precision=2) == rendered  # determinism
...
def test_config_rejects_invalid_values(kwargs: dict[str, Any]) -> None:
    """``BaseLLMPredictorConfig`` validates bounds on construction."""
    with pytest.raises(ValidationError):
        BaseLLMPredictorConfig(**kwargs)
...
def test_predict_end_to_end_with_mocked_sampler(svc: DataService, task: ForecastingTask) -> None:
    ...
    n_samples = 8
    cfg = BaseLLMPredictorConfig(model="anthropic/claude-sonnet-4-5", n_samples=n_samples, cache=False)

    # Build N slightly-varied length-H trajectories so quantiles have spread.
    rng = np.random.default_rng(42)
    trajectories = [(150.0 + rng.normal(0, 2.0, HORIZON)).tolist() for _ in range(n_samples)]
    sampler_return = (trajectories, 0.008, 800, 400, 0)

    with (
        patch("methods.base_llmp._bootstrap_litellm"),
        patch("methods.base_llmp._sample_trajectories", return_value=sampler_return),
    ):
        predictor = BaseLLMPredictor(cfg)
        preds = predictor.predict(task, svc.context(AS_OF))

    assert len(preds) == 1, "Single-horizon task → one Prediction."
    pred = preds[0]
    assert pred.predictor_id == "base_llmp[anthropic/claude-sonnet-4-5]"
...
def test_predict_multi_horizon_emits_one_prediction_per_step(svc: DataService) -> None:
    """Multi-horizon task returns one ``Prediction`` per step; each uses the right slice."""
    ...
    cfg = BaseLLMPredictorConfig(n_samples=16, cache=False)

    rng = np.random.default_rng(0)
    n_steps = 6
    trajectories = [(200.0 + rng.normal(0, 3.0, n_steps)).tolist() for _ in range(16)]
    sampler_return = (trajectories, 0.0, 0, 0, 0)

    with (
        patch("methods.base_llmp._bootstrap_litellm"),
        patch("methods.base_llmp._sample_trajectories", return_value=sampler_return),
    ):
        preds = BaseLLMPredictor(cfg).predict(multi_task, svc.context(AS_OF))

    assert [p.forecast_date for p in preds] == [
        (pd.Timestamp(AS_OF) + pd.DateOffset(months=h)).to_pydatetime() for h in [3, 6]
    ]
    # Sanity: medians match what ``np.quantile`` gives at each slice.
...
def test_predict_raises_when_all_samples_malformed(svc: DataService, task: ForecastingTask) -> None:
    ...
    cfg = BaseLLMPredictorConfig(n_samples=4, cache=False)
    # Sampler returns no valid trajectories + non-zero parse_failures.
    sampler_return: tuple[list[list[float]], float, int, int, int] = ([], 0.0, 0, 0, 4)
