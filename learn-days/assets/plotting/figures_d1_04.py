...
def _crps_by_horizon(nb_text: str) -> dict[str, list[float]]:
    ...
    table = next(
        (b for b in blocks if "BY PREDICTOR" in b.upper() and "h=5" in b), None
    )
    if table is None:
...
def fig_leakage_crps_by_horizon() -> None:
    ...
    pre_text = subprocess.run(
        ["git", "show", f"{PREFIX_COMMIT}:implementations/energy_oil_forecasting/"
         "04_systematic_backtest_eval.ipynb"],
        cwd=REPO, capture_output=True, text=True, check=True,
    ).stdout
    pre = _crps_by_horizon(pre_text)

    for src, tag in ((pre, f"pre-fix ({PREFIX_COMMIT})"), (post, "post-fix")):
        missing = [k for k in (AGENT_LABEL, "AutoARIMA", "Naive (Last Value)")
                   if k not in src]
        if missing:
    ...
    ax.plot(x, naive, color=vp.MUTED, lw=1.6, ls=(0, (1, 2)), marker="o", ms=3,
            label="Naive", zorder=2)
    ax.plot(x, arima, color=vp.BLUE, lw=1.8, ls=(0, (4, 2)), marker="o", ms=3.5,
            label="AutoARIMA", zorder=3)
    # The agent, honest: also fans out.
    ax.plot(x, agent_post, color=vp.PINK, lw=2.4, marker="o", ms=4.5,
            label="News agent — post-fix", zorder=5)
    # The agent, leaked: flat — the fingerprint.
    ax.plot(x, agent_pre, color=vp.RED, lw=2.4, ls=(0, (5, 2)), marker="s", ms=4.5,
            label="News agent — pre-fix (leaked)", zorder=6)

    # Call out the flat line and the h=21 gap (= the size of the cheating).
    ax.annotate("flat — the leak's fingerprint",
                xy=(21, agent_pre[2]), xytext=(10.3, agent_pre[2] - 2.4),
                fontsize=10.5, color=vp.RED, fontweight="bold", va="top")
    ax.annotate(
        "", xy=(21, agent_post[2]), xytext=(21, agent_pre[2]),
        arrowprops=dict(arrowstyle="<->", color=vp.INK, lw=1.3))
    ax.annotate(f"+{pct:.0f}% CRPS\nwhen un-leaked",
                xy=(21, (agent_pre[2] + agent_post[2]) / 2),
                xytext=(15.4, (agent_pre[2] + agent_post[2]) / 2),
                fontsize=10.5, color=vp.INK, fontweight="bold", va="center")
    ...
    ax.margins(x=0.02)
    ax.legend(loc="upper left", fontsize=10, ncol=1, framealpha=0.9,
              facecolor="white", edgecolor="none")
    vp.save(fig, f"{SESSION}/leakage_crps_by_horizon", slot="figure")
    print(f"[fig] leakage_crps_by_horizon  (pre All={all_pre}, post All={all_post}, "
          f"+{pct:.0f}%)")
...
def fig_agentic_system() -> None:
    ...
    def box(x, y, w, h, *, fc, ec, title, sub=None, fs=12, sub_fs=10,
            lw=1.6, ls="-", title_c=vp.INK, sub_c=vp.BODY):
        ax.add_patch(FancyBboxPatch(
        ...
        ax.text(x + w / 2, cy, title, ha="center", va="center", fontsize=fs,
                color=title_c, fontweight="bold", zorder=3)
        if sub:
            ax.text(x + w / 2, y + h / 2 - h * 0.27, sub, ha="center",
                    va="center", fontsize=sub_fs, color=sub_c, zorder=3)

    def arrow(x1, y1, x2, y2, *, color=vp.INK, lw=1.8, cs=None, ls="-"):
        ax.add_patch(FancyArrowPatch(
            (x1, y1), (x2, y2), arrowstyle="-|>", mutation_scale=13, lw=lw,
    ...
    box(24, 79, 74, 15, fc="#FFF6E6", ec=vp.AMBER,
        title="Harness — injects the cutoff (as_of)",
        sub="invisible to the LLM: it can't be omitted or spoofed",
        fs=12, sub_fs=10, sub_c=vp.BODY)

    # Main row — the four LLM/tool components (left → right request flow).
    row_y, row_h = 40, 18
    box(2, row_y, 20, row_h, fc="#FDE9F4", ec=vp.PINK,
        title="Analyst agent", sub="root · orchestrates")
    box(27, row_y, 20, row_h, fc="#F3E9FB", ec=vp.PURPLE,
        title="News sub-agent", sub="retrieves context")
    box(52, row_y, 20, row_h, fc="#FFFFFF", ec=vp.INK,
        title="search_web", sub="grounded Google", fs=11.5)
    box(77, row_y, 21, row_h, fc="#EAF8FB", ec=vp.CYAN,
        title="Verifier", sub="a different model")
    ...
    for lx, lab in ((24.5, "delegates"), (49.5, "queries"), (74.5, "raw hits")):
        ax.text(lx, 60.5, lab, ha="center", va="center", fontsize=9.5,
                color=vp.MUTED)
    ...
    ax.text(72, 71.5, "retry ×3, else fail-loud", ha="center",
            va="center", fontsize=9.5, color=vp.RED, fontweight="bold")
    ...
    ax.text(50, 27.6, "verified context returns", ha="center", va="center",
            fontsize=10, color=vp.GREEN, fontweight="bold")

    # The forecast the analyst finally emits.
    box(2, 8, 20, 15, fc="#E9F7F1", ec=vp.GREEN,
        title="Prediction", sub="calibrated forecast", fs=11.5)
