"""Script to generate 07_interactive_session.ipynb."""
...
cells.append(
    code(
        ...
        "# TRAINED_STRATEGY_DIR: the strategy after the self-directed study session\n"
        "# Change to _SKILLS_ROOT / 'wti-strategy' to demo the untrained baseline.\n"
        "DEMO_STRATEGY_DIR = TRAINED_STRATEGY_DIR\n"
        "\n"
...
# ── Section 1: The four message types ─────────────────────────────────────────
cells.append(
    md(
        "---\n"
...
cells.append(
    md(
        ...
        "\n"
        "The default agent uses `wti-strategy/` (untrained baseline). To demo the\n"
        "trained variant, you can temporarily copy the trained `skill_state.yaml` over:\n"
        "\n"
        "```bash\n"
        "# Use the trained strategy for the web demo\n"
        "cp adaptive_agent/skills/wti-strategy-trained/skill_state.yaml \\\n"
...
out = Path("implementations/energy_oil_forecasting/07_interactive_session.ipynb")
