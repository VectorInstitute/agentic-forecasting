...
cells.append(
    code(
        ...
        "\n"
        "# Collect h=21d predictions per (predictor, origin)\n"
        "_TARGET_HORIZON = 21\n"
        "_h21: dict[tuple[str, str], object] = {}\n"
        "for name, result in all_eval_results.items():\n"
        "    for pred in result.predictions:\n"
        "        horizon = (pd.Timestamp(pred.forecast_date) - pd.Timestamp(pred.as_of)).days\n"
        "        if abs(horizon - _TARGET_HORIZON) <= 2 and isinstance(pred.payload, ContinuousForecast):\n"
        "            _h21[(name, str(pred.as_of.date()))] = pred\n"
        "\n"
