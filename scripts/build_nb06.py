...
cells.append(
    md(
        ...
        "\n"
        "One panel per predictor, all eight eval origins on a shared time axis.\n"
        ...
        "for every origin.  Dashed verticals mark the origin dates."
    )
)
...
cells.append(
    code(
        ...
        "# (handles cases where exact h=21d may be h=20 or h=22 depending on calendar)\n"
        ...
        "                _h21[key] = pred\n"
        "print(f'h-max predictions collected: {len(_h21)} ({len(_h21) // max(len(_PREDICTOR_ORDER), 1)} origins per predictor)')\n"
        "\n"
        "_origins = sorted({str(pred.as_of.date()) for result in all_eval_results.values() for pred in result.predictions})\n"
        "\n"
        "# Price window: span all origins plus context on each side\n"
        ...
        "        showlegend=show_leg, legendgroup='actual',\n"
        "    ), row=row_idx, col=1)\n"
        "\n"
        ...
        "\n"
        "fig.update_layout(\n"
        "    title='h=21d forecasts across all 2026 eval origins — point (diamond) + 80% CI (bar)',\n"
        "    height=220 * _n_rows,\n"
        "    width=950,\n"
        "    showlegend=False,\n"
...
# ── Section 8: State integrity check ──────────────────────────────────────────
cells.append(
    md(
        "---\n"
