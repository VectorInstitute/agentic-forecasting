...
cells.append(
    code(
        ...
        "print('\\nKey CRPS deltas:')\n"
        "for a, b, label in [\n"
        "    ('Agent — untrained', 'AutoARIMA', 'news search alone (untrained vs AutoARIMA)'),\n"
        "    ('Agent — Act 2b (news)', 'Agent — untrained', 'news-grounded training (Act 2b vs untrained)'),\n"
        "    ('Agent — Act 2b (news)', 'Agent — Act 2a (stats)', 'news in training (Act 2b vs Act 2a)'),\n"
        "]:\n"
        "    if a in _crps and b in _crps:\n"
        "        delta = _crps[b] - _crps[a]\n"
        "        print(f'  {label}: {delta:+.3f}')"
    )
)
