"""Script to regenerate 01_intro_agentic_predictor.ipynb."""
import json
...
cells = [
    md(
        "# WTI Crude Oil Price Forecasting — Introducing the Agentic Predictor\n"
        "\n"
        ...
        "comparable and can slot into the systematic backtest in Notebook 2.\n"
        "\n"
        ...
        "| 4 | `AgentPredictor` (code+news) | Code-executing agent — Gemini native code execution + 3 forecasting skills |"
    ),
    ...
    code(
        ...
        "\n"
        "# Make the implementations directory importable so we can use energy_oil_forecasting.*\n"
        "sys.path.insert(0, str(Path.cwd().parent))\n"
        "\n"
        "import pandas as pd\n"
        "\n"
        "from energy_oil_forecasting.data import build_wti_service, WTI_SERIES_ID\n"
        "\n"
        "# ── Data service ──────────────────────────────────────────────────────────\n"
    ...
    md(
        ...
        "The same wrapper will be used in Notebook 2's backtest loop."
    ),
    ...
    code(
        ...
        "\n"
        "print(f'LLMP forecast (no tools):\\n')\n"
        "for p in llmp_preds:\n"
        "    fc = p.payload\n"
        "    print(\n"
    ...
    md(
        ...
        "\n"
        "2. **Three forecasting skills** — ADK `SkillToolset` provides reference data\n"
        "   on demand:\n"
        "   - `rolling-statistics` — pre-computed WTI volatility benchmarks\n"
        "   - `trend-projection` — copy-pasteable sklearn patterns with CI formulas\n"
        "   - `forecast-visualization` — matplotlib template for visual sanity checks\n"
        "\n"
    ...
    code(
        ...
        "                yerr=[np.array(points) - np.array(lo),\n"
        "                      np.array(hi) - np.array(points)],\n"
        "                fmt='o', capsize=5, linewidth=2, label='Point + 80% CI')\n"
        "    if actual_val:\n"
    ...
    md(
        ...
        "   backtesting in Notebook 2 possible.\n"
        "\n"
        ...
        "\n"
        "3. **Skills provide reference data, not code.** The three ADK skills\n"
        "   (`rolling-statistics`, `trend-projection`, `forecast-visualization`) each\n"
        "   contain one real file in `references/` that the agent loads on demand.\n"
        "   Following the design rule in `docs/adk-skills-guide.md`, no scripts are\n"
        "   present and the instruction explicitly forbids `run_skill_script`.\n"
        "\n"
        ...
        "\n"
        "→ **Notebook 2** runs a full 2025 backtest across all predictors, selects\n"
...
out = Path(__file__).parent / "01_intro_agentic_predictor.ipynb"
